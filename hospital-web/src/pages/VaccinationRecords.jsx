...
const VaccinationRecords = () => {
  ...
  const getStatus = (vaccineId) => {
    ...
    return found
      ? { status: "Completed", date: found.dateAdministered }
      ...
      <div className="flex-1 ml-64 p-8">
        <header className="mb-8">
          ...
          <p className="text-gray-500">
            Search for a child to view or update their history.
          </p>
        </header>
            ...
            <div className="bg-gradient-to-r from-blue-50 to-white p-6 rounded-2xl border border-blue-100 flex justify-between items-center">
              <div>
                <h2 className="text-2xl font-bold text-gray-800">
                  {childData.childName || "Newborn"}
                </h2>
                <p className="text-gray-500">Parent: {childData.parentName}</p>
                <div className="flex gap-4 mt-2 text-sm text-gray-600">
                  <span className="flex items-center gap-1">
                    <Calendar size={14} /> DOB: {childData.dob}
                  </span>
                  <span className="flex items-center gap-1">
                    🩸 Blood: {childData.bloodGroup}
                  </span>
                </div>
              </div>
              ...
              <div className="divide-y divide-gray-100">
                {VACCINE_SCHEDULE.map((vaccine) => {
                  ...
                  return (
                    <div
                      key={vaccine.id}
                      className={`p-5 flex items-center justify-between hover:bg-gray-50 transition-colors ${isCompleted ? "bg-green-50/30" : ""}`}
                    >
                      ...
                      <div className="flex items-center gap-6">
                        {isCompleted ? (
                          ...
                          <button
                            onClick={() =>
                              markAsCompleted(vaccine.id, vaccine.name)
                            }
