...
const StatCard = ({ icon, label, value, color }) => (
  <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex items-center gap-4">
    <div className={`p-4 rounded-xl ${color} bg-opacity-10`}>
      {React.cloneElement(icon, { className: `w-8 h-8 ${color.replace('bg-', 'text-')}` })}
    </div>
    <div>
  ...
  useEffect(() => {
    const fetchStats = async () => {
      ...
      try {
        ...
        const q = query(collection(db, "children"), where("hospitalId", "==", auth.currentUser.uid));
    ...
    <div className="flex bg-background min-h-screen">
      ...
      <div className="flex-1 ml-64 p-8">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-12 text-center">
          ...
          <h3 className="text-lg font-semibold text-gray-800">No appointments scheduled</h3>
